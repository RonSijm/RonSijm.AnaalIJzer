...
src/
  ...
  Tools/RonSijm.AnaalIJzer.Ridder/    — unified RazorConsole and headless command front end
...
- Ridder's headless command mode and interactive TUI are presentation modes. They must create `ToolRequest` values and execute them through `ToolRunner`; do not duplicate MSBuild loading, config parsing or output generation in either mode.
- Add operations and aliases to `ToolOperationCatalog`. The Ridder command list and TUI operation selector must continue to derive from that catalog.
- Update `ToolingTests` whenever an operation or supported input kind changes.
...
| Build Ridder | `dotnet build src\Tools\RonSijm.AnaalIJzer.Ridder` |
