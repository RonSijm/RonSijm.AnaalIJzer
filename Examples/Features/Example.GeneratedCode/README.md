...
Generated source is quiet by default because generated files are often large, volatile, and owned by another tool. This example opts in only `Generated_Clock_Kitchen.g.cs`, which deliberately reads `DateTime.UtcNow` and raises one `ARCH021`.
