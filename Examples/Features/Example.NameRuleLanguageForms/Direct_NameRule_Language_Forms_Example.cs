...
{
    // ARCH_NAME_008: compound assignment still moves animalId into fruitId.
    ...
    // ARCH_NAME_008 twice: deconstruction compares corresponding source and target components.
    public void SwapIds(int fruitId, int animalId)
    {
        (fruitId, animalId) = (animalId, fruitId);
    }

    // ARCH_NAME_008: conversions and conditional branches keep their source meaning.
    public int ChooseFruitId(bool chooseAnimal, int fruitId, int animalId)
    {
        return chooseAnimal ? (int)animalId : fruitId;
    }

    // ARCH_NAME_008: expression-bodied returns have the method-return site.
    public int GetFruitId(int animalId) => animalId;

    // ARCH_NAME_008 twice: named arguments follow their resolved parameter names.
