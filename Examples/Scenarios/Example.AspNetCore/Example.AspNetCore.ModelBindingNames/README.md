...
`DoctorId patientId` and `PatientId doctorId` are valid C#, but convention-based HTTP/model binding can still treat the misleading parameter names as meaningful. The configured `RequireDeclarationNameMatchesType` rule therefore reports two `ARCH008` findings.
