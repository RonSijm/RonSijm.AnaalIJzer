...
`PizzaOrdersController` delegates directly and is valid. `PizzaOrderPreviewController` returns the correct response shape without calling the owner, so it reports `ARCH023`.
