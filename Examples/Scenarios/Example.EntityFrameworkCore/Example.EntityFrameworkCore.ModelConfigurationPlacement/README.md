...
`PizzaOrderConfiguration` is correctly placed. `PreviewPizzaOrderConfiguration` implements the same EF Core interface but lives in `Application/Mapping/`, so it produces one `ARCH_SRC_007`.
