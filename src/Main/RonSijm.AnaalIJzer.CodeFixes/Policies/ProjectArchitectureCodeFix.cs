...
internal static class ProjectArchitectureCodeFix
{
    ...
    internal static async Task TryRegisterAsync(CodeFixContext context, Diagnostic diagnostic)
    {
        ...
        var sourceProjectName = ConfigurationCodeFixSupport.ReadStringProperty(diagnostic, ArchitecturalDiagnostics.PropertySourceProjectName);
        var targetProjectName = ConfigurationCodeFixSupport.ReadStringProperty(diagnostic, ArchitecturalDiagnostics.PropertyTargetProjectName);
        ...
        var selectedTitle = $"Allow project '{sourceProjectName}' to reference '{targetProjectName}'";
        context.RegisterCodeFix(
            CodeAction.Create(
                selectedTitle,
                cancellationToken => ConfigurationCodeFixEditor.EditConfigurationAsync(
                    context.Document,
                    configurationSource,
                    document => TryAddSelectedAllowedProjectReference(document, sourceProjectGroup, targetProjectGroup, sourceProjectName, targetProjectName),
                    cancellationToken),
                selectedTitle),
            diagnostic);
    }
    ...
    private static bool TryAddSelectedAllowedProjectReference(
        XDocument document,
        ...
        string targetProjectName)
    {
        ...
        var existingAllowed = projectArchitecture.Elements("AllowedProjectReference")
            .Any(element =>
                string.Equals(element.Attribute("from")?.Value, sourceProjectGroup, StringComparison.Ordinal)
                && string.Equals(element.Attribute("to")?.Value, targetProjectGroup, StringComparison.Ordinal)
                && element.Elements("From").Any(selector => string.Equals(selector.Attribute("exactName")?.Value, sourceProjectName, StringComparison.Ordinal))
                && element.Elements("To").Any(selector => string.Equals(selector.Attribute("exactName")?.Value, targetProjectName, StringComparison.Ordinal)));
        if (existingAllowed)
        {
            return false;
        }
        ...
        var newElement = new XElement(
            ...
            new XAttribute("from", sourceProjectGroup),
            new XAttribute("to", targetProjectGroup),
            new XElement("From", new XAttribute("exactName", sourceProjectName)),
            new XElement("To", new XAttribute("exactName", targetProjectName)));
