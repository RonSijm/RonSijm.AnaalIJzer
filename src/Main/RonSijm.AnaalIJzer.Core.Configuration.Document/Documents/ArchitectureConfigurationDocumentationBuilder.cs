...
public static class ArchitectureConfigurationDocumentationBuilder
{
	...
	private static string GetDocumentationLabel(XElement element)
	{
		var result = element.Name.LocalName switch
		{
			...
			"GeneratedCode" => "Generated source analysis (" + (element.Attribute("mode")?.Value ?? "Exclude") + ")",
			"Path" when element.Parent?.Name.LocalName == "GeneratedCode" => "Generated source path " + (GetMatcherDisplayName(element) ?? "(no matcher)"),
			...
			"Assembly" when element.Parent?.Parent?.Name.LocalName == "AssemblyReferencePolicy" => "Assembly reference " + (GetMatcherDisplayName(element) ?? "(no matcher)"),
			...
			"SolutionTopology" => "Solution topology",
			"Module" => element.Attribute("name")?.Value ?? "Module",
			...
			"AllowedModuleReference" => $"{element.Attribute("from")?.Value ?? "?"} -> {element.Attribute("to")?.Value ?? "?"}",
			"BlockedModuleReference" => $"{element.Attribute("from")?.Value ?? "?"} -x-> {element.Attribute("to")?.Value ?? "?"}",
			"From" => "Project " + (GetMatcherDisplayName(element) ?? "(no matcher)"),
			"To" => "Project " + (GetMatcherDisplayName(element) ?? "(no matcher)"),
			...
			"AssemblyReferencePolicy" => $"Assembly reference policy for {element.Attribute("projectGroup")?.Value ?? "?"}",
			"AssemblyAttributePolicy" => "Assembly attribute policy",
			"Attribute" when element.Parent?.Parent?.Name.LocalName == "AssemblyAttributePolicy" => "Assembly attribute " + (GetMatcherDisplayName(element) ?? "(no matcher)"),
			"Argument" when element.Parent?.Parent?.Parent?.Name.LocalName == "AssemblyAttributePolicy" => "Assembly attribute argument " + (GetMatcherDisplayName(element) ?? "(no matcher)"),
			...
			"NamespaceHierarchyPolicy" => "Namespace hierarchy for " + (element.Attribute("rootNamespace")?.Value ?? "(missing root namespace)"),
			"BlockedRelation" => "Block " + (element.Attribute("relation")?.Value ?? "(missing relation)"),
			"AllowedReturn" => "Allowed direct return shapes",
			"ForbiddenOperations" => "Forbidden selected operations",
			"ForbiddenOperation" => "Forbidden operation",
			"BehavioralOperations" => "Behavioral operation policies",
			"Operations" => "Explicit operation contracts",
			"Operation" => "Operation " + (element.Attribute("name")?.Value ?? "(unnamed)"),
			"Owner" => "Operation owner",
			"Request" => "Operation request type",
			"Response" => "Operation response type",
			"EntryPoint" when element.Parent?.Name.LocalName == "Operation" => "Operation entry point",
			"RequiredOperation" => "Required operation",
			"RequiredOperationBefore" => "Required operation before another operation",
			"ForbiddenOperationAfter" => "Forbidden operation after another operation",
			"MaximumOperationCount" => "Maximum operation count",
			"DeclarationMatcher" => "Owning declaration selector",
			"BeforeOperation" => "Operation that must come after the required operation",
			"AfterOperation" => "Terminal operation",
			"OperationMatcher" => "Operation " + (element.Attribute("kind")?.Value ?? "(no kind)"),
			"ContainingType" => "Containing type " + (GetMatcherDisplayName(element) ?? "(all types)"),
			"Member" => "Member " + (GetMatcherDisplayName(element) ?? "(all members)"),
			...
			"Invocation" when element.Parent?.Name.LocalName == "AllowedReturn" => "Allowed returned invocation " + (GetMatcherDisplayName(element) ?? "(any invocation)"),
			"New" when element.Parent?.Name.LocalName == "AllowedReturn" => "Allowed returned object creation " + (GetMatcherDisplayName(element) ?? "(any object creation)"),
			"Identifier" when element.Parent?.Name.LocalName == "AllowedReturn" => "Allowed returned identifier " + (GetMatcherDisplayName(element) ?? "(any identifier)"),
			"MemberAccess" when element.Parent?.Name.LocalName == "AllowedReturn" => "Allowed returned member access " + (GetMatcherDisplayName(element) ?? "(any member access)"),
			"Literal" when element.Parent?.Name.LocalName == "AllowedReturn" => "Allowed returned literal " + (GetMatcherDisplayName(element) ?? "(any literal)"),
