...
public static class ArchitectureConfigurationMatcherReader
{
	...
	public static bool IsMatcherAttribute(string name)
	{
		var result = name is
	...
	private static void TryAddCondition(XElement element, string attributeName, MatchKind kind, ImmutableArray<MatchCondition>.Builder conditions)
	{
		...
		conditions.Add(new MatchCondition(kind, value));
	}
}
