...
public static partial class ArchitecturalConfigParser
{
	...
	private static ImmutableArray<ReturnValueRule> ParseReturnValueRules(IEnumerable<XElement> ruleElements, string xmlPath, ImmutableArray<ConfigurationIssue>.Builder issues)
	{
		...
		foreach (var element in ruleElements)
		{
			...
			if (element.Attributes().Any(attribute => !IsReturnValueRuleAttribute(attribute.Name.LocalName, target)))
			{
				continue;
			}
	...
	private static bool IsReturnValueRuleAttribute(string name, CodeObservationMatchTarget target)
	{
		var result = name is "description" or "comment" or "typeName" or "exactName" or "exactFullName"
