...
internal static partial class ArchitectureDocumentationMarkdownBuilder
{
	...
	private static void AppendProjectArchitecture(StringBuilder sb, AnalyzerConfig config)
	{
		...
		if (rules.Length > 0)
		{
			...
			sb.AppendLine();
			sb.AppendLine("| Rule | Edge | Project selectors | Description |");
			sb.AppendLine("|------|------|-------------------|-------------|");
			for (var itemIndex = 0; itemIndex < config.Documentation.Items.Length; itemIndex++)
			{
				var rule = config.Documentation.Items[itemIndex];
				if (rule.Kind is not ("AllowedProjectReference" or "BlockedProjectReference"))
				{
					continue;
				}

				var mode = rule.Kind == "AllowedProjectReference" ? "Allowed" : "Blocked";
				var selectors = GetProjectReferenceSelectors(config.Documentation.Items, itemIndex);
				sb.AppendLine($"| {mode} | `{EscapeTable(rule.Label)}` | {EscapeTable(selectors)} | {EscapeTable(rule.Description ?? string.Empty)} |");
			}
		...
		var assemblyReferencePolicies = config.Documentation.Items.Where(item => item.Kind == "AssemblyReferencePolicy").ToArray();
		if (assemblyReferencePolicies.Length > 0)
		{
			sb.AppendLine("### Assembly Reference Policies");
			sb.AppendLine();
			sb.AppendLine("These policies are evaluated by workspace tooling against direct raw MSBuild `Reference` items. They do not produce compiler `ARCHxxx` diagnostics.");
			sb.AppendLine();
			sb.AppendLine("| Project group | Mode | Assembly identity matcher | Description |");
			sb.AppendLine("|---------------|------|---------------------------|-------------|");

			for (var itemIndex = 0; itemIndex < config.Documentation.Items.Length; itemIndex++)
			{
				var policy = config.Documentation.Items[itemIndex];
				if (policy.Kind != "AssemblyReferencePolicy")
				{
					continue;
				}
				...
				for (var containerIndex = itemIndex + 1; containerIndex < config.Documentation.Items.Length; containerIndex++)
				{
					...
					for (var matcherIndex = containerIndex + 1; matcherIndex < config.Documentation.Items.Length; matcherIndex++)
					{
						...
						if (matcher.Depth != container.Depth + 1 || matcher.Kind != "Assembly")
						{
							continue;
						}

						sb.AppendLine($"| `{EscapeTable(projectGroup)}` | {EscapeTable(mode)} | `{EscapeTable(matcher.Label)}` | {EscapeTable(matcher.Comment ?? matcher.Description ?? container.Description ?? policy.Description ?? string.Empty)} |");
					}
				}
			}
	...
	private static string GetProjectReferenceSelectors(ImmutableArray<ArchitectureDocumentationItem> items, int ruleIndex)
	{
		var rule = items[ruleIndex];
		var selectors = new List<string>();
		foreach (var item in items.Skip(ruleIndex + 1))
		{
			if (item.Depth <= rule.Depth)
			{
				break;
			}

			if (item.Depth != rule.Depth + 1 || item.Kind is not ("From" or "To"))
			{
				continue;
			}

			selectors.Add($"{item.Kind}: {item.Label}");
		}

		var result = selectors.Count == 0 ? "All matching projects" : string.Join("; ", selectors);
