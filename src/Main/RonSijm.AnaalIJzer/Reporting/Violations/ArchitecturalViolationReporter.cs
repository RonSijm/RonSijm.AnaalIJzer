...
internal static class ArchitecturalViolationReporter
{
	...
	{
		var result = GenerateMarkdownReport(diagnostics.Select(TryCreateViolationRecord).OfType<ViolationRecord>(), config, assemblyName);
	...
	{
		var result = GenerateMarkdownReport(diagnostics.Select(TryCreateViolationRecord).OfType<ViolationRecord>(), config, inputName, inputLabel);
	...
	internal static string GenerateMarkdownReport(IEnumerable<ViolationRecord> violationBag, AnalyzerConfiguration config, string? inputName, string inputLabel)
	{
		...
		sb.AppendLine($"| ARCH002 — Unrecognized dependency | {arch002.Count} |");
	...
	private static ViolationRecord? TryCreateViolationRecord(Diagnostic diagnostic)
	{
		...
		{
			var result = properties.TryGetValue(key, out var value) ? value : null;
