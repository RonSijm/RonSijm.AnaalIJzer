...
internal readonly struct DependencyGraph(ImmutableArray<DependencyEdge> dependencyEdges)
{
	...
	public ImmutableHashSet<(string From, string To)> AllowedEdges =>
		DependencyEdges.Where(edge => edge.IsAllowed && edge.IsExplicit).Select(edge => (edge.From, edge.To)).ToImmutableHashSet();
	...
	public ImmutableHashSet<string> WildcardTargets =>
		DependencyEdges.Where(edge => edge.IsAllowed && edge.IsWildcardTarget).Select(edge => edge.To).ToImmutableHashSet();
	...
	public ImmutableHashSet<string> WildcardSources =>
		DependencyEdges.Where(edge => edge.IsAllowed && edge.IsWildcardSource).Select(edge => edge.From).ToImmutableHashSet();
	...
	public bool IsEdgeAllowed(string from, string to, string site) =>
		EvaluateEdge(from, to, site).IsAllowed;
	...
	{
		return EvaluateDependency(from.Layer.Name, to.Layer.Name, site);
	}
	...
	{
		return EvaluateGate(string.Empty, from, to, site);
	}
	...
	private DependencyEdgeEvaluation EvaluateGate(string scopePath, string from, string to, string site)
	{
		...
		{
			if (edge.ScopePath == scopePath && edge.IsBlocked && EdgeMatches(edge, from, to) && edge.AllowsSite(site))
			{
		...
		foreach (var edge in DependencyEdges)
		{
			if (edge.ScopePath != scopePath || !edge.IsAllowed || !EdgeMatches(edge, from, to))
			{
				continue;
			}
	...
	public bool HasEdge(string from, string to) =>
		DependencyEdges.Any(edge => edge.IsAllowed && edge.IsExplicit && edge.From == from && edge.To == to);

	public bool HasEdge(string scopePath, string from, string to) =>
		DependencyEdges.Any(edge => edge.ScopePath == scopePath && edge.IsAllowed && EdgeMatches(edge, from, to));

	public bool Matches(DependencyEdge edge, string from, string to) =>
		GetGateScopes(from, to).Contains(edge.ScopePath) && EdgeMatches(edge, from, to);
	...
	private static bool PathMatches(string actualPath, string configuredPath) =>
		actualPath == configuredPath || actualPath.StartsWith(configuredPath + "/", StringComparison.Ordinal);

	private static string FormatBoundary(string scopePath) =>
		string.IsNullOrEmpty(scopePath) ? "the root boundary" : $"boundary '{scopePath}'";
	...
	private static string JoinPath(string[] parts, int length) =>
		length <= 0 ? string.Empty : string.Join("/", parts, 0, length);
...
internal readonly struct DependencyEdgeEvaluation(bool isAllowed, string denialReason, DependencyDenialKind denialKind, string scopePath, string fromPath, string toPath)
{
	...
	public static DependencyEdgeEvaluation Denied(string reason, DependencyDenialKind denialKind, string scopePath, string fromPath, string toPath) =>
		new(false, reason, denialKind, scopePath, fromPath, toPath);
}
