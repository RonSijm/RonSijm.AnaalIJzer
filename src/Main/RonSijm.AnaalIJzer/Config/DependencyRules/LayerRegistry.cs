...
internal readonly struct LayerRegistry(
    ...
    IReadOnlyDictionary<string, LayerNode> nodesByPath,
    IReadOnlyDictionary<string, MatcherRule> forbiddenTypeNames,
    ...
    ImmutableArray<(PatternMatcher Matcher, MatcherRule Rule)> allowedTypeMatchers)
{
    ...
    {
        get { return !roots.IsDefaultOrEmpty; }
    }
    ...
    {
        return new LayerMatch(rule.Layer, layers, matcherMatches, string.IsNullOrEmpty(result) ? null : result, rule.XmlLineNumber,
            rule.XmlLinePosition, rule.XmlPath);
    }
