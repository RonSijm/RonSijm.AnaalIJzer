...
internal static class RidderCommandLine
{
	...
	{
		Console.WriteLine("Ridder");
		Console.WriteLine("Rule Inspector for Dependency Diagrams, Exceptions, and Reports");
		Console.WriteLine();
		Console.WriteLine("Usage:");
		Console.WriteLine("  ridder                  Open the interactive terminal interface.");
		Console.WriteLine("  ridder tui              Explicitly open the interactive terminal interface.");
		foreach (var operation in ToolOperationCatalog.All)
		{
			Console.WriteLine($"  ridder {operation.Usage}");
		}
		...
		Console.WriteLine("  --strategy             Config generation: snapshot or conventions. Defaults to snapshot.");
...
internal sealed record CommandOptions
{
	...
	private static ConfigurationGenerationStrategy ParseStrategy(string value)
	{
		...
		throw new CommandLineException($"Unknown generation strategy: {value}. Use snapshot or conventions.");
	}
