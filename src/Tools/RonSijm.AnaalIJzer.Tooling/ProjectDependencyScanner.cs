			...
			foreach (var node in syntaxTree.GetRoot(cancellationToken).DescendantNodes())
			{
				switch (node)
				{
					...
					case TypeDeclarationSyntax typeDeclaration:
						...
						{
							AddTypeDependency(typeDeclaration, semanticModel.GetTypeInfo(baseType.Type, cancellationToken).Type, DependencySites.Inheritance, semanticModel, resolveLayer, observations, cancellationToken);
						}
						break;
