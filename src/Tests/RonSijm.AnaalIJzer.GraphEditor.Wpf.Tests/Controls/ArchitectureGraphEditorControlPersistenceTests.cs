...
public sealed partial class ArchitectureGraphEditorControlPersistenceTests
{
	...
	[Fact]
	public void ExceptionPolicyEdit_PersistsXmlConfiguration()
	{
		RunOnStaThread(() =>
		{
			var path = WriteTempFile(
				...
				"""
				<ArchitecturalLevels>
				  <ExceptionPolicy requireOwner="true" warnBeforeDays="21" description="Review stale exceptions." />
				  <Layer name="Customer"><Class typeName="CustomerType" /></Layer>
				</ArchitecturalLevels>
				""");
			var control = CreateControl(CreateSnapshot(path, ArchitectureConfigurationSourceKind.XmlFile));
			var requireOwner = FindCheckBoxByContent(control, "requireOwner");
			var warnBeforeDays = FindTextBoxByText(control, "21");
			var policyDescription = FindTextBoxByText(control, "Review stale exceptions.");

			requireOwner.IsChecked = false;
			warnBeforeDays.Text = "30";
			warnBeforeDays.RaiseEvent(new RoutedEventArgs(UIElement.LostFocusEvent));
			policyDescription.Text = "Exceptions need explicit cleanup windows.";
			policyDescription.RaiseEvent(new RoutedEventArgs(UIElement.LostFocusEvent));
			DrainDispatcher();

			var content = File.ReadAllText(path);
			content.Should().Contain("<ExceptionPolicy");
			content.Should().NotContain("requireOwner=\"true\"");
			content.Should().Contain("warnBeforeDays=\"30\"");
			content.Should().Contain("description=\"Exceptions need explicit cleanup windows.\"");
		});
	}

	[Fact]
	public void ExceptionPolicyEdit_PersistsInlineAssemblyMetadataConfiguration()
	{
		RunOnStaThread(() =>
		{
			var path = WriteInlineConfigurationFile(
				"""
				<ArchitecturalLevels>
				  <ExceptionPolicy requireReason="true" warnBeforeDays="14" description="Temporary exceptions need a reason." />
				  <Layer name="Customer"><Class typeName="CustomerType" /></Layer>
				</ArchitecturalLevels>
				""");
			var control = CreateControl(LoadInlineSnapshot(path));
			var requireReason = FindCheckBoxByContent(control, "requireReason");
			var warnBeforeDays = FindTextBoxByText(control, "14");
			var policyDescription = FindTextBoxByText(control, "Temporary exceptions need a reason.");

			requireReason.IsChecked = false;
			warnBeforeDays.Text = "45";
			warnBeforeDays.RaiseEvent(new RoutedEventArgs(UIElement.LostFocusEvent));
			policyDescription.Text = "Temporary exceptions need owners and dates.";
			policyDescription.RaiseEvent(new RoutedEventArgs(UIElement.LostFocusEvent));
			DrainDispatcher();

			var content = File.ReadAllText(path);
			content.Should().Contain("<ExceptionPolicy");
			content.Should().NotContain("requireReason=\"true\"");
			content.Should().Contain("warnBeforeDays=\"45\"");
			content.Should().Contain("description=\"Temporary exceptions need owners and dates.\"");
		});
	}
