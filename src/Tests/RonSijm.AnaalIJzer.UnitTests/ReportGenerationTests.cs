...
public sealed class ReportGenerationTests
{
	...
	public async Task Analyzer_DoesNotWriteReportOrDocumentationFiles()
	{
		...
		try
		{
			var configPath = Path.Combine(tempDir, "ArchitecturalLevels.xml");
			...
			File.Exists(documentationPath).Should().BeFalse("Ridder owns documentation generation");
		}
		finally
		{
	...
	public async Task ViolationReporter_RendersAnalyzerDiagnostics()
	{
		...
		report.Should().Contain("| ARCH002 — Unrecognized dependency (strict) | 1 |");
		...
		var strictDiagnostic = diagnostics.Single(diagnostic => diagnostic.Id == ArchitecturalDiagnosticIds.UnrecognizedDependency);
		strictDiagnostic.Properties[ArchitecturalDiagnostics.PropertyCallerTypeName].Should().Be("PatientManager");
		strictDiagnostic.Properties[ArchitecturalDiagnostics.PropertyCallerLayerName].Should().Be("Manager");
		strictDiagnostic.Properties[ArchitecturalDiagnostics.PropertyDepTypeName].Should().Be("MysteryTopping");
	}
	...
	public void DocumentationGenerator_RendersDescriptionsWildcardsAndEscapesMermaidLabels()
	{
		var config = ParseConfig("""
		                         <ArchitecturalLevels description="Rules for the pizzeria">
		                             <Allowed description="Only approved dependency names.">
		                                 <Class startsWith="Order" description="Order types are approved." />
		                             </Allowed>
		...
		markdown.Should().Contain("Any([\"* (any layer)\"])");
		...
		markdown.Should().Contain("| Allowed | `global` | `Class startsWith=\"Order\"` | Order types are approved. |");
	...
	{
		var additionalText = new TestAdditionalText("ArchitecturalLevels.xml", config);

		return ArchitecturalConfigParser.Parse(
