...
public sealed class NameMatcherTests
{
	...
	[Fact]
	public async Task Regex_LowerPrecedenceThanContains()
	{
		// Both 'contains' and 'regex' would match, but 'contains' wins per precedence rules.
		// We assert this by checking the displayed forbidden-rule label (which uses the
		...
		forbidden.Should().ContainSingle();
		forbidden[0].GetMessage(CultureInfo.InvariantCulture).Should().Contain("Legacy");
	}
