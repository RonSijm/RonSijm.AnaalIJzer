...
internal sealed class ExampleProjectAnalysisHost : IDisposable
{
    ...
    private readonly ProjectAnalysisHost _projectAnalysisHost;

    public ExampleProjectAnalysisHost()
    {
        RegisterMsBuild();
        _projectAnalysisHost = new ProjectAnalysisHost("Release");
    }
    ...
    {
        var analysis = await _projectAnalysisHost.AnalyzeAsync(projectPath, cancellationToken);
