...
public sealed class ExamplesIntegrationTests
{
	...
	[
		...
		ExpectXml("Diagnostics/Example.Arch001.NonConstructorInjection", (ArchitecturalDiagnosticIds.IllegalLevelDependency, 8)),
		...
		ExpectXml("Features/Example.AllowedSites", (ArchitecturalDiagnosticIds.IllegalLevelDependency, 24)),
		Expect("Features/Example.AllowedTypes", (ArchitecturalDiagnosticIds.ForbiddenDependency, 1)),
		ExpectXml("Features/Example.ArchitectureHealth"),
		...
		ExpectXml("Features/Example.IncludeSettings", (ArchitecturalDiagnosticIds.IllegalLevelDependency, 1)),
		...
		ExpectXml("Features/Example.NestedLayers", (ArchitecturalDiagnosticIds.IllegalLevelDependency, 3)),
		...
		ExpectXml("Scenarios/Example.RepositoryQuerySurface", (ArchitecturalDiagnosticIds.IllegalLevelDependency, 2)),
		ExpectXml("Documentation/Example.DocumentationDemo"),
	...
	public async Task ExampleProjects_BuildWithExpectedAnalyzerDiagnostics()
	{
		...
		ValidateExampleXmlConfigs(repositoryRoot, schemaPath, failures);
		...
		try
		{
			foreach (var expectation in Expectations)
			{
				...
				var fileConfigPath = Path.Combine(projectDirectory, "ArchitecturalLevels.xml");
				var inlineSettingsPath = Path.Combine(projectDirectory, "Properties", "AnaalIJzerSettings.cs");
				if (File.Exists(inlineSettingsPath))
				{
					failures.Add($"{expectation.RelativeProjectPath}: inline settings should live in Example.cs for simple examples, or ArchitecturalLevels.xml for broader examples; remove {inlineSettingsPath}.");
				}
				...
				else
				{
					if (!File.Exists(fileConfigPath))
					{
						failures.Add($"{expectation.RelativeProjectPath}: broader examples should use ArchitecturalLevels.xml.");
					}
				}
			}
		}
	...
	private static void ValidateExampleXmlConfigs(string repositoryRoot, string schemaPath, List<string> failures)
	{
		var examplesPath = Path.Combine(repositoryRoot, "Examples");
		foreach (var xmlPath in Directory.EnumerateFiles(examplesPath, "*.xml", SearchOption.AllDirectories).OrderBy(path => path, StringComparer.OrdinalIgnoreCase))
		{
			...
			var relativePath = Path.GetRelativePath(repositoryRoot, xmlPath);
			ValidateXmlContent(relativePath, content, xmlPath, schemaPath, requireSchemaHint: true, failures);
		}
	}
	...
	private static ExampleBuildExpectation Expect(string projectName, params (string Id, int Count)[] diagnostics) =>
		new(projectName, ExampleConfigStyle.InlineInExample, diagnostics.ToDictionary(diagnostic => diagnostic.Id, diagnostic => diagnostic.Count, StringComparer.Ordinal));

	private static ExampleBuildExpectation ExpectXml(string projectName, params (string Id, int Count)[] diagnostics) =>
		new(projectName, ExampleConfigStyle.XmlFile, diagnostics.ToDictionary(diagnostic => diagnostic.Id, diagnostic => diagnostic.Count, StringComparer.Ordinal));
	...
	private static string FormatDiagnostics(IReadOnlyDictionary<string, int> diagnostics) =>
		diagnostics.Count == 0 ? "<none>" : string.Join(", ", diagnostics.OrderBy(pair => pair.Key, StringComparer.Ordinal).Select(pair => $"{pair.Key}={pair.Value}"));
